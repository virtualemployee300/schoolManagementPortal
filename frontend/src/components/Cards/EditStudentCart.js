...
export default function AddStudentCart() {
  ...
  //These state to store the form value
         ...
         const getStudent=async()=>{
             const data={id:id}
             const res = await axios.post("http://localhost:3000/api/studentGetData",data);
         ...
         useEffect(()=>{
          ...
          const updateStudent=async()=>{
            ...
            await axios({
                    method: "post",
                    url: "http://localhost:3000/api/studentUpdate",
                    data: formData,
      ...
      <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
        <div className="rounded-t bg-white mb-0 px-6 py-6">
          <div className="text-center flex justify-between">
            <h6 className="text-blueGray-700 text-xl font-bold">Edit Student Details</h6> 
        ...
        <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
          <form>
            ...
            {msg===true?<span className="p-5">Student Update Successfully</span>:""}
            <button
              className="bg-lightBlue-500 text-white active:bg-lightBlue-600 font-bold uppercase text-xs px-4 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none mr-1 ease-linear transition-all duration-150"
              type="button"
              onClick={(e)=>updateStudent()}
            >
              Add
            </button>
            </div>
